...
def _iter_tenant_scopes():
    tenants = registry.list_tenants()
    if tenants:
        for t in tenants:
            if not t.get("ativo", True):
                continue
            yield t.get("slug"), t.get("pg_database")
    else:
        yield None, None
...
class BackgroundService:
    ...
    def _loop_sync(self):
        time.sleep(5)
        while True:
            for slug, pg_db in _iter_tenant_scopes():
                try:
                    with tenant_scope(slug, pg_db):
                    ...
                    log_service.registrar_log(
                        f"Falha sync ({slug or 'default'}): {exc}",
                        origem="SYNC",
                        nivel="ERRO",
                    )
                ...
                try:
                    with tenant_scope(slug, pg_db):
                        ...
                        if agendamento_email.agendamento_esta_vencido(cfg):
                            ...
                            log_service.registrar_log(
                                f"E-mail agendado ({slug or 'default'}): "
                                f"{resultado.get('total_notas')} notas.",
                                origem="EMAIL",
                            )
                except Exception as exc:
                    log_service.registrar_log(
                        f"Falha e-mail ({slug or 'default'}): {exc}",
                        origem="EMAIL",
                        nivel="ERRO",
                    )
        ...
        while True:
            for slug, pg_db in _iter_tenant_scopes():
                try:
                    ...
                    log_service.registrar_log(
                        f"Falha suporte ({slug or 'default'}): {exc}",
